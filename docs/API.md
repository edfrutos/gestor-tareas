...
| --- | --- | --- | --- | --- |
| POST | `/v1/auth/login` | — | `{ username, password }` (`username` acepta email, case-insensitive) | `200 { token, user:{id,username,email,role} }` · `401 { error: "Usuario o contraseña incorrectos" }` |
...
| GET | `/v1/auth/me` | Bearer | — | `200 { id, username, email, role, ... }` |
