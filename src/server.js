...
async function main() {
  ...
  // Crear servidor HTTP o HTTPS según certificados
  let server;
  ...
  if (useHttps) {
    try {
      ...
      logger.info("[server] HTTPS enabled with SSL certificates");
    } catch (err) {
      logger.warn({ err: err.message }, "[server] SSL certificates not found, using HTTP");
